...
function blockSwapHtml(s) {
  ...
  if (s.swapWanted) {
    // Cercul „?" rămâne mereu. Dacă AM oferit aici, i se lipește pe interior
...
export async function initPlanner(mount) {
  ...
  // ROTIREA TELEFONULUI SCHIMBĂ GEOMETRIA. `rowPx()` se calculează din lățimea
  // rămasă, iar la întors ecranul acea lățime se dublează — fără asta orarul
  // rămânea desenat pentru ecranul dinainte până la prima atingere. Amânat cu
  // un cadru: la rotire, lățimea se stabilizează abia după ce se așază bara de
  // navigare.
  let reasez = null;
  const laRotire = () => {
    clearTimeout(reasez);
    reasez = setTimeout(() => { if (S.root) render(); }, 120);
  };
  window.addEventListener("resize", laRotire);
  window.addEventListener("orientationchange", laRotire);
