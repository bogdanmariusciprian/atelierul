...
import {
  ...
  pendingExplanations, approveExplanation, rejectExplanation,
...
export function renderCommunity(basePath = "") {
  ...
  const state = {
    ...
    bk: { lectie: null, kind: null, text: "", staged: [], banca: [], incarc: false,
          nota: "", notaBuna: false,
    ...
    explanationQueue: [],  // explicații propuse de elevii de la meditații, în așteptare
  ...
  function moderationBox() {
    ...
    const reportCard = (r) => {
      ...
      return `<div class="cx-repcard">
          <a class="cx-repcard__go" href="${basePath}teste/${encodeURIComponent(it.exam || "admitere-drept")}/?item=${encodeURIComponent(it.id)}" title="Deschide itemul în grila de itemi">
  ...
  function adminTabDownloads(doarExamenul = null) {
    ...
    const explanations = state.explanationQueue.filter((pr) => !cat || pr.item.exam === cat.slug);
    const explanationCard = (pr) => {
      const it = pr.item;
      ...
      return `<div class="cx-repcard">
          ...
          <div class="cx-repcard__say">
            <span class="cx-repcard__lab">Explicația propusă de ${escapeHtml(pr.pupil)}</span>
    ...
    const explanationsBlock = `<div class="cx-box">
        <div class="cx-admin__head"><h3>✍️ Explicații propuse de elevi · ${explanations.length}</h3></div>
        ${explanations.length
  ...
  function improspateazaJosul() {
    const s = socoteala(state.bk.staged);
    ...
    const deTrimis = state.bk.staged.filter((r) => !r.gata);
  ...
  async function incarcaBanca(slug) {
    ...
    state.bk.staged = state.bk.staged.map((r) => ({ ...r, gata: inBanca.has(bankCheia(r.body)) }));
  ...
  function deschideTabla() {
    ...
    if (state.bk.lectie !== slug) {
      ...
      state.bk = {
        ...
        text: "", staged: [], banca: [], incarc: true, nota: "", notaBuna: false,
  ...
  async function loadFeed() {
    try {
      ...
      if (isAdmin()) {
        ...
        state.explanationQueue = await pendingExplanations();
  ...
  mount.addEventListener("click", (e) => {
    ...
    switch (action) {
      ...
      case "bk-adauga": {
        ...
        const inBanca = new Set((state.bk.banca || []).map((x) => bankCheia(x.body)));
        const { randuri, sarite } = randuriDinLista(text, { kind, inTabel: state.bk.staged, inBanca });
        state.bk.notaBuna = false;
        if (!randuri.length && !sarite) { state.bk.nota = "Scrie măcar un cuvânt."; return render(); }
        state.bk.staged = [...state.bk.staged, ...randuri];
        state.bk.text = "";
      ...
      case "bk-coloana": {
        ...
        state.bk.staged = state.bk.staged.map((r) => r.gata ? r : {
      ...
      case "bk-scoate":
        state.bk.staged = state.bk.staged.filter((_, i) => i !== Number(btn.dataset.i));
        return render();
      ...
      case "bk-goleste":
        ...
        return render();
      case "bk-importa": {
        const deTrimis = state.bk.staged.filter((r) => !r.gata && r.tags.length);
        state.bk.notaBuna = false;
        ...
        bankAddRows(state.adminLesson, deTrimis.map((r) => ({
          ...
          state.bk.staged = state.bk.staged.filter((r) => !intrate.has(bankCheia(r.body)));
          await incarcaBanca(state.adminLesson);
          const faraBifa = state.bk.staged.filter((r) => !r.gata && !r.tags.length).length;
      ...
      case "explan-approve": {
        ...
        btn.disabled = true;
        approveExplanation(id, text).then((r) => {
          if (r && r.error) { btn.disabled = false; showToast(`N-am publicat: ${r.error}`); return; }
          state.pendingExplanations = state.explanationQueue.filter((pr) => pr.id !== id);
  ...
  mount.addEventListener("change", (e) => {
    ...
    if (ce === "bk-bifa") {
      ...
      state.bk.staged = state.bk.staged.map((r, k) => k !== i ? r : {
        ...r, tags: el.checked ? [...new Set([...r.tags, et])] : r.tags.filter((t) => t !== et),
      });
      const deTrimis = state.bk.staged.filter((x) => !x.gata);
    ...
    if (ce === "bk-nivel") {
      ...
      state.bk.staged = state.bk.staged.map((r, k) => k === i ? { ...r, level: n } : r);
      return;
    }
    ...
    if (ce === "bk-nivel-tot") {
      ...
      if (!n) return;
      state.bk.staged = state.bk.staged.map((r) => r.gata ? r : { ...r, level: n });
      return render();
    }
    if (ce === "bk-fel") {
      ...
      if (state.bk.staged.length) return render();
