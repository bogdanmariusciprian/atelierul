 ...
 * momente deosebite în același ecran.
 *
 * Acum vin toate, cu data de la care țin, iar fiecare ecran alege singur orarul
 * potrivit pentru ZIUA pe care o arată. Așa merge și o schimbare de orar căzută
 * în mijlocul săptămânii, care n-ar fi mers nicicum altfel.
 *
 * Sunt vreo șaizeci și cinci de rânduri pe an. Nu se pune problema greutății.
 */
export async function fetchOrarul() {
  return cuPlasa("orarul", async () => {
    ...
    return randuri.map(({ zi, period, clasa, sala, valabil_de_la, eticheta }) => ({
      zi, period, clasa, sala, deLa: valabil_de_la, eticheta: eticheta || "",
    }));
  }, []);
}
